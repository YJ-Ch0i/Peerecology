...
function addSchool()
{
   var inputCheckQuestions = ""
      inputCheckQuestions = parent.document.getElementById('add_school_page').innerHTML;
   ...
   for(var i=0; i<chk_box.length; i++)
      ...
      if(chk_box[i].checked == true)
         {
         var splitChk_box = chk_box[i].value.split('_');
         inputCheckQuestions += "<input type=\"hidden\" name=\"hidden_sch_code\" id=\"hidden_sch_code\" value=\""+splitChk_box[0]+",\">"
                        + "<input type=\"text\" class=\"input-md form-control\" name=\"sch_name\" value=\""+splitChk_box[1]+"\" readonly>"
                        + "<p></p>";
         }
      }
   parent.document.getElementById('add_school_page').innerHTML = inputCheckQuestions;
   alert('추가되었습니다.');
}
</script>
<!-- CSS -->
...
<body class="appear-animate">
   ...
   <div class="page" id="top">
         ...
         <tr>
         <td><input type="checkbox" name="inlineCheckbox1" value="<%=schList.get(i).getSCID()%>_<%=schList.get(i).getName() %>"></td>
